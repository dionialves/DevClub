...
async function updateConversionRate(){
    ...
    const data = await fetch("http://economia.awesomeapi.com.br/json/last/USD-BRL,USD-GBP,USD-EUR").then(response => response.json())

    coin.real.conversionRate = data.USDBRL.high
    coin.euro.conversionRate = data.USDGBP.high
    coin.libra.conversionRate = data.USDEUR.high
    console.log(coin)
}
